...
Despite the facts presented above, system coordination is not all the burden on the communications side, the main task of this system is to replicate virtual machine images among the several nodes, so the network is tasked with carrying a lot of data.
Every iMI transferred by the replication module have the possibility to be encrypted or to travel the network in plain text.
