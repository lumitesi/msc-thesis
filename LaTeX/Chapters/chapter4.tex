...
The libraries we created to implement the common set of functionalities required for the replication module, do require more libraries, implemented by the Python community. An inventory with the library dependencies is presented in Listing~\ref{listing:impl_icbd_pipfile}. These are necessary to handle communication between nodes, algorithms for data compression, and secure data transmission; the most relevant ones are described through the remaining text in this section.
...
Inside the replication module the iMI, as presented in Section~\ref{sub:icbd_imi}, is treated as a first-class citizen, being represented by the class \textit{icbdSnapshot}. This object stores the relevant metadata and properties of an iMI that are essential to unequivocally identify the multiple images present in the system; but it does not hold actual data.
...
Table~\ref{tab:impl_dvs} lists the relationship between the VDS port groups and the FCT NOVA VLANs (which are managed by an external entity – the \textit{Divisão de Infraestruturas Informáticas}).
