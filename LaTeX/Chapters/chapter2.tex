...
Either way, the challenge lays in the fact that the hypervisor needs to execute the guests OS instructions in a safe manner and at the same time provide possible different machine configurations to each of them. These characteristics, such as the number and architecture of virtual CPUs (\textit{vCPU}), the amount and type of memory available (\textit{vRAM}), the allowed space to store files (\textit{vDisk}), and so on, are user configurable but is the hypervisor that is tasked to do all the management and load balancing. The settings of all these components are compiled in a VM configuration file. In the case of VMware hypervisors, the file employs the \texttt{.vmx} extension.\cite{VMWare_VMFiles,Portnoy2012} In a KVM environment, the configuration is handled in a \texttt{.xml} file.~\cite{chirammal2016}
