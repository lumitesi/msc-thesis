...
There are too many file systems of this genre to be here listed. Nevertheless, some of the most renowned may be mentioned. As the industry-standard File Allocation Table (FAT)~\cite{wiki_File_Allocation_Table}, the New Technology File System (NTFS)~\cite{wiki_NTFS} from Microsoft, the Apple's Hierarchical File System Plus (HFS+)~\cite{wiki_HFS_Plus} also called Mac OS Extended and the B-tree file system (BTRFS) initially designed by Oracle.
	% 
