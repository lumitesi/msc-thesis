...
The life cycle of an iMI encompasses all the stages that an iMI takes throughout its course within the platform. From creation, passing through live deployment in use by multiple clients and finally its decommission and temporary or cold storage.
...
During its stay on the platform, an iMI can present four states:
...
\begin{description}
	...
	%
	\item [Not Published] This status symbolises that the image is ready to be used but has not yet been published and visible to platform users, which is particularly interesting for testing the iMI. This methodology allows for the possibility to ensure that the changes made have been applied successfully and that the image can be made available for general use.
	%
	\item [Deployed]
\end{description}
