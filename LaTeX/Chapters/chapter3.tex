...
\end{figure}
% TODO - Adicionar em Client support layer - OBS
...
\begin{description}
	\item [VM Template files] The main component is the virtual machine template in the form of a read-only image. As described in section~\ref{sub:vm_storage} the anatomy of a template follows the standard VMware and KVM formats either with multiple files (i.e., Virtual Disk Files like \texttt{.vmdk} or \texttt{.qcow}) or a \textit{raw} storage format.
	%
...
\end{figure}
% TODO - Mudar Img
% Fazer loop no Administraçao e colocar lá a fraze boot em creat
% not pub passa a Staging 
% Deployed passa published
