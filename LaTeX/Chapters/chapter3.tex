...
From an end-user perspective, the only layer that is visible and interactive is the boot layer.
However, there is more than the eye can not see, the is the need to resort to multiple services to successfully boot a client's workstation with an iCBD Machine Image.
First, the process starts with a network boot where the Dnsmasq server provides the DHCP and TFTP services needed to start a traditional PXE environment boot. The DHCP provides an IP address and the instructions to contact a TFTP server that host the PXE boot menu files. After this initial communication, a friendly looking graphical menu displays to the user an assortment of choices.
